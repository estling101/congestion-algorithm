    ...
    for subdir, _, files in os.walk(root_dir):
        for fname in files:
            if fname.endswith(".json"):
                ...
                # Step 2: Extract edies cell and new algorithm cell runtimes (NB!!! Change edies_cell and our_algo_cell at top of file if reordering the notebook)
                runtime_6 = next((d["execution_time_sec"] for d in data if d["cell_index"] == edies_cell), None)
                runtime_7 = next((d["execution_time_sec"] for d in data if d["cell_index"] == our_algo_cell), None)
...
# df has columns ['file', 'edies', 'runtime_cell_7', 'abs_diff_sec']
...
avg_row = {
    ...
    'abs_diff_sec': overall_avg_abs_diff.round(1),
    'percentage_diff': overall_avg_pct_diff.round(1)
}
